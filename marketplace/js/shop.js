...
/**
 * 상점 아이템 구매 (메타 트랜잭션 사용)
 */
async function purchaseShopItem(item) {
    try {
        Utils.showNotification('메타 트랜잭션 준비 중...', 'info');

        // 서버 지갑 주소 (환경 변수에서 가져오거나 하드코딩)
        const serverWallet = CONFIG.SERVER_WALLET_ADDRESS || '0xa5ab6C8C0560d51Db844182e286a380916Eb1487';
        ...
        if (response.mintTxHash) {
            setTimeout(() => {
                Utils.showNotification(
                    `NFT 민팅: ${Utils.shortenAddress(response.mintTxHash)}`,
                    'info'
                );
            }, 4000);
        }
